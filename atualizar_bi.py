#!/usr/bin/env python3
"""
Atualiza o BI Fogaca (index.html) com dados frescos da Meta Ads API.
...
ACCOUNT = os.getenv('META_AD_ACCOUNT_ID', 'act_950037393138268')
...
PRODUTOS = [
    ('Encontro de Negocios', ['ENCONTRO DE NEG', 'EDN']),
    ('Sales Academy', ['SALES ACADEMY']),
    ('A Mesa', ['A MESA']),
    ('Casal de Negocios', ['CASAL DE NEG']),
    ('GCR', ['GCR', 'EVENTO-GCR']),
    ('Diagnostico', ['DIAGNOSTICO', 'DIAGNÓSTICO', 'DIAGNOST']),
    ('Missao China', ['MISSAO CHINA', 'MISSÃO CHINA']),
    ('Protocolo', ['PROTOCOLO']),
]
...
def _parse_response(r):
    """Parseia response. Em caso de falha, retorna dict com 'error' contendo status + snippet do corpo."""
    body = r.text or ''
    try:
        data = r.json()
    except ValueError:
        snippet = body[:200].replace('\n', ' ')
        return {'error': {'message': f'HTTP {r.status_code} non-JSON: {snippet!r}', 'status': r.status_code}}
    if isinstance(data, dict) and 'error' in data:
        err = data['error']
        # propaga subcode/code se houver, ajuda diagnostico
        msg = f'HTTP {r.status_code} {err.get("type","")} code={err.get("code","")} subcode={err.get("error_subcode","")}: {err.get("message","")}'
...
def api_get(endpoint, params):
    ...
    for attempt in range(3):
        try:
            ...
            log(f'  Request error (tent {attempt+1}): {last_err}')
            if attempt < 2: time.sleep(5)
            continue
        data = _parse_response(r)
        if 'error' in data:
            last_err = data['error']['message']
            log(f'  API error (tent {attempt+1}): {last_err}')
...
def detectar_bloco(objective, nome):
    ...
    nome_up = (nome or '').upper()
    if 'COMPRA' in nome_up or 'VENDAS' in nome_up: return 'Vendas'
...
def fetch_campaigns_daily(camp_meta_by_id=None):
    ...
    data = api_get(f'{ACCOUNT}/insights', {
        'level': 'campaign',
        'fields': 'campaign_id,impressions,clicks,spend,actions,results',
        'time_range': json.dumps({'since': PERIODO_INICIO, 'until': until}),
        'time_increment': 1, 'limit': 500
    })
    if 'error' in data: log(f'  ERRO: {data["error"]["message"]}'); return []
...
def main():
    log('=' * 60)
    log('INICIO ATUALIZACAO BI FOGACA')
    log('=' * 60)
    ...
    any_data = False
    if daily: html = replace_var(html, 'DAILY_DATA', daily); any_data = True
    if camp_meta: html = replace_var(html, 'CAMPAIGNS_META', camp_meta); any_data = True
    if camp_daily: html = replace_var(html, 'CAMPAIGNS_DAILY', camp_daily); any_data = True
    if ad_meta: html = replace_var(html, 'ADS_META', ad_meta); any_data = True
    if ad_daily: html = replace_var(html, 'ADS_DAILY', ad_daily); any_data = True
    if ad_vid: html = replace_var(html, 'ADS_VIDEO_TOTALS', ad_vid); any_data = True

    if not any_data:
        log('Nenhum dado obtido da Meta API — abortando sem mexer no HTML.')
        sys.exit(1)
